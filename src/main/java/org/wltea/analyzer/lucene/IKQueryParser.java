...
public final class IKQueryParser {
    ...
    private static Query optimizeQueries(List<Query> queries) {
        ...
        if (queries.size() == 0) {
            ...
            BooleanQuery mustQueries = builder.build();
            return mustQueries;
        }
    }
    ...
    public static Query parse(String field, String query) {
        ...
        if (qParts.length > 1) {
            ...
            for (String q : qParts) {
                ...
                if (partQuery != null &&
                        (!(partQuery instanceof BooleanQuery) || !((BooleanQuery) partQuery).clauses().isEmpty())) {
     ...
     * @return
     */
    public static Query _parse(String field, String query) {
        if (field == null) {
            throw new IllegalArgumentException("parameter \"field\" is null");
        }
        ...
        List<Query> toQueries(String fieldName) {
            ...
            if (acceptedBranchs != null && acceptedBranchs.size() > 0) {
                if (acceptedBranchs.size() == 1) {
                    ...
                    BooleanQuery build = orQuery.build();
    ...
    static class ExpressionParser {
        ...
        private Query toQuery(Element op) {
            ...
            if ('&' == op.type) {
                if (q1 != null) {
                    addQuery(resultQuery, q1, Occur.MUST);
                }

                if (q2 != null) {
                    addQuery(resultQuery, q2, Occur.MUST);
                }

            } else if ('|' == op.type) {
                if (q1 != null) {
                    addQuery(resultQuery, q1, Occur.SHOULD);
                }

                if (q2 != null) {
                    addQuery(resultQuery, q2, Occur.SHOULD);
                }
        ...
        private Query luceneQueryParse(String fieldName, String keyword) {
            ...
            QueryParser luceneQueryParser = new QueryParser(fieldName, new IKAnalyzer());
            try {
    ...
    private static void addQuery(BooleanQuery.Builder resultQuery, Query q1, Occur must) {
        if (q1 instanceof BooleanQuery) {
            List<BooleanClause> clauses = ((BooleanQuery) q1).clauses();
            if (!clauses.isEmpty()
                    && clauses.get(0).getOccur() == must) {
                for (BooleanClause c : clauses) {
                    resultQuery.add(c);
                }
            } else {
                resultQuery.add(q1, must);
            }
